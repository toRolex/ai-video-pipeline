...
class TestInitCreatesTable:
    ...
    def test_init_creates_table(self, store):
        conn = store._conn()
        cursor = conn.execute("PRAGMA table_info(video_metrics)")
        ...
        indexes = [
            row["name"]
            for row in idx_cursor.execute(
                "SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='video_metrics'"
            ).fetchall()
        ]
        idx_cursor.close()
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        job = {
            ...
            "asset_bundle": {
                ...
                "selected_clips": [
                    {
                        "file_path": "/fake/clip1.mp4",
                        "asset_id": "a1",
                        "trim_start": 0,
                        "trim_duration": 5,
                    },
                    {
                        "file_path": "/fake/clip2.mp4",
                        "asset_id": "a2",
                        "trim_start": 2,
                        "trim_duration": 8,
                    },
