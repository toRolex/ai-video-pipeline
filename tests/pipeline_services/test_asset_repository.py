...
def test_increment_usage(repo):
    record = AssetRecord(asset_id="test_003", file_path="/d.mp4", category=Category.MACRO, product="见手青")
    repo.insert(record)
...
def test_disabled_assets_not_returned(repo):
    record = AssetRecord(asset_id="test_004", file_path="/d.mp4", category=Category.MACRO, product="见手青")
    repo.insert(record)
    repo.update_status("test_004", "disabled")
    results = repo.query_by_category("见手青", Category.MACRO)
    assert len(results) == 0


def test_usage_sort_order(repo):
    for i in range(3):
        repo.insert(AssetRecord(asset_id=f"a{i}", file_path=f"/d{i}.mp4", category=Category.MACRO, product="见手青"))
